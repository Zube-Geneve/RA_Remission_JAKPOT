#Version history
#
# v8.1 Initial creation from CDAI v8.0_use_fine_tuned_parameters AND use top 5 features
#
# v9.1 Draw ROC Plots
#
# v10.1 Change the positive class to 1 
#
# v11.1 Applying the changes for ROC plots to test and external data
#
# v12.0 Changing the selection of patients as only the patients with at least one CDAI info during follow-up visit 
#       between 6 to 24 months after treatment start 
# 
# v13.0 Applying SHAP as well as using the dummies.
#
# v14.0 Apply treatment group as dummy.
#
...
setwd("C:/Users/zubey/OneDrive/Documents/Projects/32. ML for JAKPOT/CDAI_2.8_Short")
...
write.csv(jakpot.data_eff1, file="Outputs - Fine_tuning/jakpot_data_eff1.csv")
write.csv(jakpot.data_eff1_baseline, file="Outputs - Fine_tuning/jakpot_data_eff1_baseline.csv")
write.csv(remission_cdai, file="Outputs - Fine_tuning/remission_cdai.csv")
...
columns <- c("PGA",
             ...
             "Pain",
             "AntiCCPevernever",
             "Code", #We need to have Code because we will separate CH 
             # Outcome #
             "CDAI_response"
)

# Create predictors by removing the outcome column
predictors <- setdiff(columns, "CDAI_response")

# Select the features using the column names
combined_dataset <- combined_dataset[, ..columns]
...
write.csv(continuous_to_categorical, "Outputs - Fine_tuning/continuous_to_categorical.csv", row.names = FALSE)
...
write.csv(combined_dataset_conv, file="Outputs - Fine_tuning/combined_dataset_conv.csv")
...
#ONE_HOT ENCODING
# STEP 1: Specify variables to one-hot encode (from your list)
vars_to_encode <- character(0)
...
# Only run encoding if there are variables to encode
...
# Save
write.csv(combined_dataset_conv, file="Outputs - Fine_tuning/combined_dataset_conv_apres_exc.csv", row.names = FALSE)
...
write.csv(external_data, file="Outputs - Fine_tuning/external_data.csv")
write.csv(internal_data, file="Outputs - Fine_tuning/internal_data.csv")
...
write.csv(train_data, file="Outputs - Fine_tuning/train_data.csv")
write.csv(test_data, file="Outputs - Fine_tuning/test_data.csv")
...
# Combine the data
combined_data_T1 <- rbind(train_data, test_data, external_data)
...
# Create Table 1
table1 <- CreateTableOne(strata = "Dataset", data = combined_data_T1)
...
writeLines(captured_output, "Outputs - Fine_tuning/table1.txt")
####
...
outputs_path <- "C:/Users/zubey/OneDrive/Documents/Projects/32. ML for JAKPOT/CDAI_2.8_Short/Outputs - Fine_tuning/"
tool_path <- "C:/Users/zubey/OneDrive/Documents/Projects/32. ML for JAKPOT/CDAI_2.8_Short/Tool/"
...
# Write a reference to the plot file in your text file
...
write.csv(shap_values, file = "Outputs - Fine_tuning/shap_values_training_feature.csv", row.names = FALSE)

# Plot SHAP summary to PDF
pdf("Outputs - Fine_tuning/SHAP_Summary_Plot_training_feature.pdf")
...
pdf("Outputs - Fine_tuning/SHAP_Summary_Plot_training_feature_top110.pdf", width = 8, height = 6)
...
# Save SHAP summary to file
write.csv(shap_values, "Outputs - Fine_tuning/shap_values_finetuned_optimal_threshold.csv", row.names = FALSE)

# Plot SHAP summary to PDF
pdf("Outputs - Fine_tuning/SHAP_Summary_Plot_finetuned_optimal_threshold.pdf")
shap.plot.summary(shap_values)
...
# 4. Plot only these 10
pdf("Outputs - Fine_tuning/SHAP_Summary_Plot_finetuned_optimal_threshold_top10.pdf", width = 8, height = 6)
shap.plot.summary(shap_top10)
